...
function setCard(prefix, hazard) {
  byId(`${prefix}Score`).textContent = hazard?.score ?? '--';
...
function plot(data) {
  for (const eq of data.earthquake?.events || []) {
    ...
    L.circleMarker([eq.lat, eq.lng], {
      ...
      weight: 1,
      fillOpacity: .72
    }).addTo(map).bindPopup(popup(`M${eq.magnitude} earthquake`, [eq.place, new Date(eq.time).toLocaleString()]));
  }
...
function renderEvents(data) {
  ...
  for (const t of data.tsunami?.messages || []) rows.push({
    rank: /WARNING/i.test(t.title) ? 95 : /ADVISORY/i.test(t.title) ? 75 : /WATCH/i.test(t.title) ? 60 : 25,
    type: 'TSUNAMI', severity: t.level || 'MESSAGE', title: t.title, text: t.updated ? `Updated ${new Date(t.updated).toLocaleString()}` : 'NOAA warning-center message'
  });
  for (const v of data.volcano?.volcanoes || []) {
    if (!['WARNING','WATCH','ADVISORY'].includes((v.alertLevel || '').toUpperCase())) continue;
    rows.push({ rank: v.alertLevel === 'WARNING' ? 90 : v.alertLevel === 'WATCH' ? 70 : 45, type: 'VOLCANO', severity: v.alertLevel, title: v.name, text: v.synopsis || 'USGS volcano notice' });
  }
  for (const e of (data.earthquake?.events || []).filter(e => e.magnitude >= 4.5).slice(0, 8)) rows.push({
    rank: Math.round((e.magnitude || 0) * 10), type: 'EARTHQUAKE', severity: `M${e.magnitude}`, title: e.place, text: new Date(e.time).toLocaleString()
  });
...
async function load() {
  try {
    ...
    byId('overallScore').textContent = data.index?.score ?? '--';
    byId('overallLabel').textContent = scoreLabel(data.index?.score ?? 0);
    ...
    const degraded = Object.entries(data.sources || {}).filter(([,v]) => !v.ok).map(([k]) => k);
    byId('sourceState').textContent = degraded.length ? `Degraded sources: ${degraded.join(', ')}. Missing data is not treated as safe.` : 'All configured upstream sources responded successfully.';
    plot(data);
    renderEvents(data);
  } catch (error) {
